...
public class TalkPrintServices {
    private static final String MODULE = TalkPrintServices.class.getName();
    private static final String RESOURCE = "TalkUiLabels";

    public static Map<String, Object> printReportPdf(DispatchContext dctx, Map<String, ? extends Object> context) {
        Locale locale = (Locale) context.get("locale");
        String screenLocation = "component://talk/widget/talk/TalkReportScreens.xml";
        String reportScreenName = "TalkReport";
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "TalkGeneralErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (IOException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "TalkIOErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        // put the talkId in the screen context, is a parameter coming into the service
        //reportScreenRenderer.getContext().put("talkId", context.get("talkId"));
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "TalkSAXErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (ParserConfigurationException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "TalkParserConfigurationErrorRenderingScreen", UtilMisc.toMap("errorString",
                    e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        */
        ...
        if (services.length > 0) {
            ...
            try {
                ...
                String errMsg = UtilProperties.getMessage(RESOURCE, "TalkUnableToPrintPDFFromXSL-FO", UtilMisc.toMap("errorString",
                        pe.toString()), locale);
